...
pub const Kind = enum { exec, replay };
...
pub const Config = union(Kind) {
    ...
    /// Replay receives terminal data externally (no PTY, no subprocess).
    replay: termio.Replay.Config,
};
...
pub const Backend = union(Kind) {
    ...
    pub fn initTerminal(self: *Backend, t: *terminal.Terminal) void {
        switch (self.*) {
            ...
            .replay => |*replay| replay.initTerminal(t),
        }
    }
    ...
    pub fn threadEnter(
        ...
        switch (self.*) {
            ...
            .replay => |*replay| try replay.threadEnter(alloc, io, td),
        }
    }
    ...
    pub fn threadExit(self: *Backend, td: *termio.Termio.ThreadData) void {
        switch (self.*) {
            ...
            .replay => |*replay| replay.threadExit(td),
        }
    }
    ...
    pub fn focusGained(
        ...
        switch (self.*) {
            ...
            .replay => |*replay| try replay.focusGained(td, focused),
        }
    }
    ...
    pub fn resize(
        ...
        switch (self.*) {
            ...
            .replay => |*replay| try replay.resize(grid_size, screen_size),
        }
    }
    ...
    pub fn queueWrite(
        ...
        switch (self.*) {
            ...
            .replay => |*replay| try replay.queueWrite(alloc, td, data, linefeed),
        }
    }
    ...
    pub fn childExitedAbnormally(
        ...
        switch (self.*) {
            ...
            .replay => |*replay| try replay.childExitedAbnormally(
                gpa,
                t,
                exit_code,
                runtime_ms,
...
pub const ThreadData = union(Kind) {
    ...
    replay: termio.Replay.ThreadData,

    pub fn deinit(self: *ThreadData, alloc: Allocator) void {
        switch (self.*) {
            .exec => |*exec| exec.deinit(alloc),
            .replay => |*replay| replay.deinit(alloc),
        }
    }
