...
pub fn init(
    ...
    // Check if this is a replay surface (no PTY, receives data externally).
    const is_replay = rt_surface.replay_mode;

    // The command we're going to execute (null for replay surfaces)
    const command: ?configpkg.Command = if (is_replay) null else command: {
        if (app.first) {
    ...
    {
        const backend: termio.Backend = if (is_replay) backend: {
            // Replay backend — no subprocess, no PTY
            break :backend .{ .replay = termio.Replay.init() };
        } else backend: {
